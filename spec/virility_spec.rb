...
describe "Virility" do
  ...
  describe "Public API testing" do
    ...
    it "Virility.counts should return a hash of counts" do
      expect(Virility.counts(@url)).to eq({:facebook=>5116303, :linkedin => 17, :pinterest=>1, :plus_one=>8, reddit:35, :stumble_upon=>4731, :twitter=>121 })
    end

    it "Virility.total should return the total count" do
      expect(Virility.total(@url)).to eq(5121216)
    end
  ...
  describe "Error Proofing" do
    ...
    it "should return 0 for all strategy counts" do
      ...
      expect(@virility.counts).to eq({:facebook=>0, :linkedin=>0, :pinterest=>0, :plus_one=>0, :reddit=>0, :stumble_upon=>0, :twitter=>0 })
    end
  end
end
